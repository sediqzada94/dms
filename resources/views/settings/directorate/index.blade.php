                ...
                <div class="card radius-10">
                    <div class="card-title d-flex justify-content-between">
                        <div class="d-flex align-items-center mb-0">
                            ...
                            <h5 class="mb-0 fc-title">{{ __('general_words.directorate') }}</h5>
                        </div>

                            <div class="mt-2">
                            <div class="blink ms-4">
                                    <i v-show="disable" class="fadeIn animated bx bx-arrow-back arrow1"></i>
                                    <btn :label="'@lang('general_words.directorate')'" @click="openModal"
                    ...
                    <div class="card-body card-body1 p-4">
                        ...
                        @include('settings.directorate.create-edit-modal')
                    </div>
                </div>
            </div>
        </div>
        ...
        let vm = new Vue({
            ...
            data() {
                return {
                    loading: false,
                    showLoading: false,
                    url: '{{route("setting.directorate.index")}}?',
                    columns: [
                        {
            ...
            methods: {
                ...
                openModal(){
                    $('#AddDirectorateModal').modal('show');
                },
                saveItems() {
                    this.save_item_loader = true;
                    axios.post("{{route('setting.directorate.store')}}", this.form).then((res) => {
                        let response = res.data;
                        if (response.status == 200) {
                            this.save_item_loader = false;
                            $('#AddDirectorateModal').modal('hide');
                            this.$emit('saveItems')
                ...
                editRecord(id = null, index) {
                   ...
                   // this.selected_dir = this.directorates.find((e) => e.id == this.directorate_id);
                    axios.get(`{!!url('setting/directorate')!!}/${id}`).then((res) => {
                        let data = res.data;
                ...
                updateItems() {
                    this.save_item_loader = true;
                    axios.patch(`{!!url('setting/directorate')!!}/${this.id}`, this.form).then((res) => {
                        let response = res.data;
                        console.log('response', response);
                        if (response.status == 200) {
                            $('#AddDirectorateModal').modal('hide');
                            showMessage(response.message, 'success');
                            this.$set(this.apiData.data, this.rowIndex, response.directorate)
                            this.resetForm();
                            this.save_item_loader = false;
                            this.isEdit = false;
                        } else {
